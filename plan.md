...
**Read:** `NAAP/astro-simulations/sun-motion-simulator/src/LatitudePicker.jsx`. Do **not** copy RS `EarthGlobeNode`/`FlatEarthMapNode`/`EarthShoreData*` (heavy Explorer-specific data). **Create** `src/sun-paths/view/LatitudePickerNode.ts` — stylized globe (gradient circle + dashed latitude lines) with draggable marker mapping y → latitude, bidirectional with `latitudeProperty`.
**Gate:** full gate + visual.
