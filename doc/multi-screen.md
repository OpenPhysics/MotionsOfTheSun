...
```
main.ts
  └─ MotionsOfTheSunScreen            (Screen<MotionsOfTheSunModel, MotionsOfTheSunScreenView>)
       ├─ MotionsOfTheSunModel         owns all state
       └─ MotionsOfTheSunScreenView    owns all visuals
```
...
Mirror the structure of `src/motions-of-the-sun-screen/`:
...
existing `MotionsOfTheSunScreen.ts`.
...
const icon = new ScreenIcon(
  new Rectangle(0, 0, 548, 373, { fill: MotionsOfTheSunColors.accentColorProperty }),
