 ...
 *
 * ── Adding content ────────────────────────────────────────────────────────────
 * 1. Create Node subclasses in separate files (e.g. SiderealSolarTimeControlPanel.ts)
...
export class SiderealSolarTimeScreenView extends ScreenView {
  public constructor(model: SiderealSolarTimeModel, options?: ScreenViewOptions) {
    ...
    // it the same way. See SiderealSolarTimeScreenSummaryContent for the four content regions.
    super({
    ...
    const placeholderText = new Text("Sidereal and Solar Time", {
