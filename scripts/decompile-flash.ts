 ...
 * Flash sources under `../Baseline/Astronomy/flash-animations/` (sibling Baseline repo), so the ported Motions of the Sun sim can be
 ...
 *
 * Run with:
 *   npm run decompile                 # the three lab simulators → NAAP/decompiled/
 *   npm run decompile -- --all        # all MOS-relevant movies
 ...
 */
const MOS_TARGETS = [
  // The published motion3 lab animations (one per screen), matched to their flashdev2 sources.
  "sunMotions/sunMotions068-C.swf", // sunmotions.swf — Motions of the Sun simulator (lab shipped 068)
  "siderealSolarTime/siderealSolarTime.swf", // siderealSolarTime.swf — Sidereal Time and Solar Time
  "zodiac/zodiac017.swf", // zodiac.swf — the Sun's motion along the zodiac
  // Supporting sun-path and sidereal-time demos from the lab pages.
  "sunPathSimulator/sunPathSimulator.swf",
  "sunPathDiagram/sunPathDiagram003.swf",
  "sunPathComponent/sunPathComponent002.swf",
...
const DEFAULT_TARGETS = MOS_TARGETS.slice(0, 3);
...
Usage:
  npm run decompile                  Decompile the three lab simulators → NAAP/decompiled/
  npm run decompile -- --all         Decompile all MOS-relevant movies
  npm run decompile -- <path>…       Decompile specific .swf / .fla / folder(s)

Flags:
  --all              Decompile the curated set of MOS-relevant SWFs
